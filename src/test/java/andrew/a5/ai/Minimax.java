...
import static andrew.a5.ai.GameModel.WIN;

import andrew.a5.ai.TranspositionTable.StateInfo;
import andrew.cms.util.maybe.Maybe;
import andrew.cms.util.maybe.NoMaybeValue;
...
public class Minimax<GameState, Move> {
    ...
    /** Whether a non-leaf game state was used for evaluating game value. */
    private boolean evaluatedNonLeaf;
    ...
    public Maybe<Move> findBestMove(GameState state, int searchTime) {
        ...
        evaluatedNonLeaf = true;
        try {
            for (int depth = 0;
                    evaluatedNonLeaf && (new Date().getTime()) - start < searchTime;
                    depth++) {
                // the best move found at this search depth
                Maybe<Move> localBestMove = bestMove;
                evaluatedNonLeaf = false;
                int best = -WIN;
                if (showSearchInfo) {
    ...
    public int search(GameState state,
        ...
        boolean leafState = gameModel.hasEnded(state);
        if (depth == 0 || leafState) {
            if (!leafState) evaluatedNonLeaf = true;
